...
export module volcano.dotenv;

export namespace volcano::dotenv {
...
namespace volcano::dotenv {
    ...
    LoadResult load_env_file(const std::filesystem::path& path, bool override_existing) {
        ...
        while (std::getline(file, line)) {
            ...
            if (detail::starts_with(trimmed, "export ")) {
                trimmed = detail::trim(trimmed.substr(7));
            }
            ...
            auto value = detail::trim(trimmed.substr(eq_pos + 1));
            ...
            auto unquoted = detail::strip_quotes(value);

            if (detail::set_env_var(key, unquoted, override_existing)) {
                result.loaded++;
            } else {
                result.skipped++;
            }
